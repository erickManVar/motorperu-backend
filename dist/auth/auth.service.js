...
let AuthService = class AuthService {
    constructor(db, betterAuthService) {
        this.db = db;
    ...
    async validateSession(request) {
        try {
            ...
            const user = await this.db
                .select()
                .from(schema.users)
                .where((0, drizzle_orm_1.eq)(schema.users.id, session.user.id))
                .limit(1);
            if (!user[0])
                return null;
            return {
...
exports.AuthService = AuthService = __decorate([
    ...
    __metadata("design:paramtypes", [Object, better_auth_service_1.BetterAuthService])
], AuthService);
